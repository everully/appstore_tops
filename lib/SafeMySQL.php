...
class SafeMySQL
{
	...
	{
		return $this->_insertID;//mysqli_insert_id($this->conn);
	}
	...
	private function rawQuery($query)
	{
        ...
        if($insertID) $this->_insertID=$insertID;
	...
	private function createSET($data)
	{
		...
		{
            $temp = explode(':', $value);
            $v = (isset($temp[1]) && $temp[1] == 'sql') ? $temp[0] : $this->escapeString($value);

			$query .= $comma.$this->escapeIdent($key).'='.$v;
			$comma  = ",";
		}
		return $query;
	}
    ...
    public function Insert($table, $object, $ignore=false, $replace=false)
        {
            $columns = array();
            $values = array();
            $args = array();
            ...
            foreach ($object as $key => $value)
            {
                ...
                elseif( isset( $temp[1] ) &&  !empty($temp[1]) )
                {
                    if( $temp[1] == 'i' )
                    {
                        $values[] = '?i';
            ...
            $sql = "{$insert} INTO ?n ($columns_s) VALUES ($values_s)";
